...
// ─── 审核审计日志 ───
const AUDIT_DIR = path_1.default.resolve(process.cwd(), 'audit-logs');
if (!fs_1.default.existsSync(AUDIT_DIR))
    fs_1.default.mkdirSync(AUDIT_DIR, { recursive: true });
function writeAuditLog(entry) {
    try {
        const date = new Date().toISOString().slice(0, 10);
        const filePath = path_1.default.join(AUDIT_DIR, `moderation-${date}.jsonl`);
...
function isEnabled() {
    ...
    if (!enabled) {
        logger_1.logger.info('AI moderation disabled (no API config) — using word-list only');
    }
    else {
        logger_1.logger.info(`AI moderation enabled: ${cfg.model}`);
    }
    return enabled;
}
// ─── 核心调用 ───
...
async function callDeepSeek(text) {
    ...
    logger_1.logger.info(`[AI MODERATION] → Request: model=${cfg.model} textLen=${text.length} sentLen=${safeText.length} text="${safeText.slice(0, 80)}"`);
    const t0 = Date.now();
    try {
        ...
        // ── 完整日志：API 返回的每一个字段 ──
        logger_1.logger.info(`[AI MODERATION] ← Response: HTTP ${res.status} (${elapsed}ms)`);
        logger_1.logger.info(`[AI MODERATION]   model: ${json.model || 'N/A'}`);
        logger_1.logger.info(`[AI MODERATION]   id: ${json.id || 'N/A'}`);
        const choice = json?.choices?.[0];
        if (choice) {
            logger_1.logger.info(`[AI MODERATION]   finish_reason: ${choice.finish_reason || 'N/A'}`);
            logger_1.logger.info(`[AI MODERATION]   message.role: ${choice.message?.role || 'N/A'}`);
            logger_1.logger.info(`[AI MODERATION]   message.content: "${choice.message?.content || ''}"`);
            logger_1.logger.info(`[AI MODERATION]   message.reasoning_content: "${choice.message?.reasoning_content || '(none)'}"`);
            if (choice.message) {
                logger_1.logger.info(`[AI MODERATION]   message keys: [${Object.keys(choice.message).join(', ')}]`);
            }
            if (choice.logprobs) {
                logger_1.logger.info(`[AI MODERATION]   logprobs: present`);
            }
        }
        else {
            logger_1.logger.warn(`[AI MODERATION]   ⚠️ No choices in response! Full body: ${JSON.stringify(json).slice(0, 500)}`);
        }
        if (json.usage) {
            logger_1.logger.info(`[AI MODERATION]   usage: prompt=${json.usage.prompt_tokens} completion=${json.usage.completion_tokens} total=${json.usage.total_tokens}`);
        }
        const msg = choice?.message;
        const output = String(msg?.content || msg?.reasoning_content || '').trim();
        logger_1.logger.info(`[AI MODERATION]   → Final output: "${output}"`);
        return output;
    }
    finally {
        clearTimeout(timeout);
...
// ─── 导出 ───
async function aiModerate(text, auditCtx) {
    ...
    try {
        ...
        // 非 0/1 → 重试一次
        logger_1.logger.warn('AI moderation: unexpected output, retrying', { raw: output });
        const t1 = Date.now();
        ...
        if (output !== '0') {
            consecutiveErrors++;
            logger_1.logger.warn('AI moderation: retry also unexpected', { raw: output, consecutiveErrors });
        }
        else {
            consecutiveErrors = 0;
        }
        ...
        logger_1.logger[level](`AI moderation ${isTimeout ? 'timeout' : 'error'} (consecutive errors: ${consecutiveErrors}/3): ${err.message}`);
        // 熔断
        if (consecutiveErrors >= 3 && !circuitOpen) {
            circuitOpen = true;
            logger_1.logger.error('AI moderation circuit OPEN — disabled for 5 minutes');
            // 通知所有管理员
            ...
            circuitTimer = setTimeout(() => {
                circuitOpen = false;
                consecutiveErrors = 0;
                logger_1.logger.info('AI moderation circuit HALF-OPEN — testing...');
