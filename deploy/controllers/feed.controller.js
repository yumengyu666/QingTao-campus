...
async function getFollowingFeed(req, res, next) {
    try {
        ...
        const type = req.query.type; // goods | post | lostfound | all
        // 获取我关注的用户ID列表
        const followings = await database_1.prisma.follow.findMany({
        ...
        // 关注用户的最新商品
        if (!type || type === 'goods') {
            const goods = await database_1.prisma.goods.findMany({
                where: {
        ...
        if (!type || type === 'lostfound') {
            const lostFounds = await database_1.prisma.lostFound.findMany({
                where: {
                    userId: { in: followingIds },
