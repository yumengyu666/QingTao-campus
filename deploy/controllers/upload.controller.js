...
function validateImageMagic(filePath, ext) {
...
async function uploadImage(req, res, next) {
    try {
        ...
        if (!files || files.length === 0)
            return (0, response_1.error)(res, '请选择图片文件');
        if (files.length > 9)
            return (0, response_1.error)(res, '单次最多上传9张图片');
        for (const file of files) {
            if (!file.mimetype.startsWith('image/'))
                return (0, response_1.error)(res, `文件 ${file.originalname} 不是有效的图片格式`);
            const ext = file.originalname.split('.').pop()?.toLowerCase();
            if (!['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].includes(ext || ''))
                return (0, response_1.error)(res, `不支持的图片格式: .${ext}`);
            ...
            if (ext && !validateImageMagic(file.path, ext)) {
                ...
                return (0, response_1.error)(res, `文件 ${file.originalname} 不是有效的图片（文件内容与扩展名不匹配）`);
            }
        }
        const results = await Promise.all(files.map(async (file) => {
            const isBanner = req.body.context === 'banner';
            const url = isBanner ? await (0, upload_service_1.compressBannerImage)(file.path) : await (0, upload_service_1.compressImage)(file.path);
            const blurredUrl = await (0, upload_service_1.createBlurredImage)(file.path);
            // 创建图片审核记录（admin 上传自动通过）
            const review = await database_1.prisma.imageReview.create({
                data: {
                    url,
                    blurredUrl,
        ...
        return (0, response_1.success)(res, { urls: results }, '上传成功');
    }
    catch (err) {
        next(err);
    }
...
async function uploadFile(req, res, next) {
    try {
        const file = req.file;
        if (!file)
            return (0, response_1.error)(res, '请选择文件');
        // 魔术字校验：拒绝扩展名与实际内容不符的文件
        const ext = file.originalname.split('.').pop()?.toLowerCase() || '';
        if (!validateMagicBytes(file.path, ext)) {
            fs_1.default.unlink(file.path, () => { }); // 删除可疑文件
            return (0, response_1.error)(res, '文件类型与扩展名不匹配，已被拒绝');
        }
        ...
        const safeName = file.originalname
            ...
            .replace(/_+/g, '_');
        return (0, response_1.success)(res, { url, originalName: safeName, size: file.size }, '上传成功');
    }
    catch (err) {
        next(err);
    }
...
async function uploadAvatar(req, res, next) {
    try {
        ...
        const file = files[0];
        if (!file.mimetype.startsWith('image/'))
            return (0, response_1.error)(res, '请上传图片文件');
        const url = await (0, upload_service_1.compressImage)(file.path);
        await database_1.prisma.user.update({
            where: { id: req.user.userId },
            data: { avatarUrl: url },
        });
        return (0, response_1.success)(res, { url }, '头像上传成功');
    }
    catch (err) {
        next(err);
    }
...
async function uploadVoice(req, res, next) {
    try {
        const file = req.file;
        if (!file)
            return (0, response_1.error)(res, '请选择语音文件');
        const relativePath = file.path.split('uploads').pop()?.replace(/\\/g, '/') || `/${file.filename}`;
        const voiceUrl = `/uploads${relativePath}`;
        return (0, response_1.success)(res, { url: voiceUrl, fileName: file.originalname, size: file.size }, '上传成功', 201);
    }
    catch (err) {
        next(err);
    }
