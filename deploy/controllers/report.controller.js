...
exports.startViolationClear = startViolationClear;
 ...
 */
async function reportMessages(req, res, next) {
    try {
        const reporterId = req.user.userId;
        const { reportedUserId, messageIds } = req.body;
        if (!reportedUserId || !Array.isArray(messageIds) || messageIds.length === 0) {
            return (0, response_1.error)(res, '请选择要举报的消息');
        }
        if (messageIds.length > 20)
            return (0, response_1.error)(res, '单次最多举报20条消息');
        if (reporterId === reportedUserId)
            return (0, response_1.error)(res, '不能举报自己');
        ...
        if (messages.length === 0)
            return (0, response_1.error)(res, '未找到要举报的消息');
        // 逐条AI审核（仅审核被举报人发送的消息）
        ...
        for (const msg of messages) {
            ...
            const result = await (0, moderation_service_1.aiModerate)(msg.content, { userId: reportedUserId, contentType: 'report' });
        ...
        if (violationCount > 0) {
            ...
            logger_1.logger.info(`[REPORT] User #${reportedUserId}: ${violationCount} violations in report, total=${newCount}${exceeded ? ' → 私信受限' : ''}`);
        }
        return (0, response_1.success)(res, {
            reviewed: messages.length,
            violations: violationCount,
            totalViolations: (await database_1.prisma.user.findUnique({ where: { id: reportedUserId }, select: { violationCount: true } }))?.violationCount || 0,
            details: results,
...
function startViolationClear() {
    const check = async () => {
        try {
            ...
            if (result.count > 0) {
                logger_1.logger.info(`[VIOLATION] Cleared private message restriction for ${result.count} users`);
            }
        }
        catch (err) {
            logger_1.logger.error(`[VIOLATION] Clear check failed: ${err.message}`);
        }
    };
