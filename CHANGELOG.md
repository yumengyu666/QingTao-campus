...
### 六大新功能模块
- **物品预约系统**: 买家预约看货→卖家确认/拒绝→24h过期释放
- **公开求购清单**: 发布求购信息(分类/校区/预算)，独立列表和详情页
- **每日签到**: 主页签到组件，7天进度条，连续签到里程碑
- **物品交换提议**: 以物易物，选择自己的物品发起交换，对方接受/拒绝
- **用户成就徽章**: 8枚预置徽章，签到/发布/交易等行为触发
- **话题标签系统**: 10个预置标签，热门标签云，标签下内容聚合

### 新增 API (20+)
- /api/reservations — 预约 CRUD + 接受/拒绝
- /api/wanted — 求购 CRUD
- /api/checkin — 签到/查询
- /api/barter — 交换提议/接受/拒绝
- /api/badges — 徽章查询
- /api/tags — 标签列表/标签下帖子

### 前端新增页面 (6)
- ReservationsPage — 预约管理（双Tab收发视图）
- WantedListPage + WantedDetailPage + PublishWantedPage
- BarterPage — 物品交换列表
- TagsPage — 标签云+标签结果
- BadgesPage — 徽章墙
