...
const templates: Record<string, (params: Record<string, string>) => { title: string; content: string }> = {
  reservation: (p) => ({
    title: `预约通知`,
...
export function getNotificationTemplate(type: string, params: Record<string, string>) {
  const tpl = templates[type];
  if (!tpl) return { title: '通知', content: '您有一条新通知' };
  return tpl(params);
}
