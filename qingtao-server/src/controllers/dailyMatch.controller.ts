...
async function isInRelationship(profileId: number): Promise<Date | null> {
  const rel = await prisma.datingRequest.findFirst({
    where: {
      status: 'accepted',
...
export async function getDailyMatch(req: Request, res: Response, next: NextFunction) {
  try {
    const profile = await prisma.datingProfile.findUnique({ where: { userId: req.user!.userId } });
    if (!profile) return error(res, '请先创建匿名身份');
    ...
    if (relDate) {
      return success(res, { matched: false, reason: 'in_relationship', relationshipDays: daysSince(relDate) });
    }
    ...
    const recentMatches = await prisma.dailyMatch.findMany({
      where: {
    ...
    // 过滤掉在恋爱中的
    const available: { id: number; userId: number }[] = [];
    ...
    if (available.length === 0) {
      return success(res, { matched: false, reason: 'no_candidates' });
    }

    // 随机选一个
    const chosen = available[crypto.randomInt(0, available.length)];
    ...
    // 通知双方
    const notify = (userId: number, peerName: string) => {
      createNotification({
        userId,
        type: 'dating_request',
        title: '今日缘分',
...
export async function revealIdentity(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!match) return error(res, '匹配不存在', 404);
    if (match.user1Id !== profile.id && match.user2Id !== profile.id) return error(res, '无权操作', 403);

    if (!match.revealed) {
      // 第一次确认 → 设置 revealed
      await prisma.dailyMatch.update({ where: { id: matchId }, data: { revealed: true } });
